...
if args.use_encoding_cache:
    ...
    aux_pos_train_texts = [split_sentences(d['aux_positive_text']) for d in train]
    aux_neg_train_texts = [split_sentences(d['aux_negative_text']) for d in train]

    if 'triplet_sample_mask' in train[0].keys():
        train_triplet_sample_masks = [d['triplet_sample_mask'] for d in train]
        aux_train_triplet_sample_masks = [d['aux_triplet_sample_mask'] for d in train]
    else:
    ...
    aux_pos_train_encodings = tokenizer(aux_pos_train_texts, truncation=True, padding=True)
    aux_neg_train_encodings = tokenizer(aux_neg_train_texts, truncation=True, padding=True)
    
    #make dataset class
    train_dataset = MultiCFClassifcationDataset(anc_train_encodings, pos_train_encodings, neg_train_encodings, aux_pos_train_encodings, aux_neg_train_encodings, train_triplet_sample_masks, train_triplet_sample_masks, train_labels)
...
model = BertForMultiCounterfactualRobustness.from_pretrained('bert-base-uncased', num_labels=NUM_LABELS)
...
for epoch in range(EPOCH_NUM):
    ...
    for batch in train_progress_bar:
        ...
        aux_pos_input_ids = batch['aux_positive_input_ids'].to(device)
        aux_pos_attention_mask = batch['aux_positive_attention_mask'].to(device)
        aux_pos_token_type_ids = batch['aux_positive_token_type_ids'].to(device)

        aux_neg_input_ids = batch['aux_negative_input_ids'].to(device)
        aux_neg_attention_mask = batch['aux_negative_attention_mask'].to(device)
        aux_neg_token_type_ids = batch['aux_negative_token_type_ids'].to(device)
       

        triplet_sample_masks = batch['triplet_sample_masks'].to(device)
        aux_triplet_sample_masks = batch['aux_triplet_sample_masks'].to(device)
        ...
        if args.use_margin_loss:
            ...
            outputs = model(
                    ...
                    neg_attention_mask, 
                    aux_pos_input_ids, 
                    aux_pos_attention_mask, 
                    aux_neg_input_ids, 
                    aux_neg_attention_mask,
                    ...
                    aux_positive_token_type_ids=aux_pos_token_type_ids,
                    aux_negative_token_type_ids=aux_neg_token_type_ids,
                    triplet_sample_masks=triplet_sample_masks, 
                    aux_triplet_sample_masks=aux_triplet_sample_masks, 
...
model = BertForMultiCounterfactualRobustness.from_pretrained(os.path.join(OUTPUT_PATH, "best_epoch"), num_labels=NUM_LABELS)
model.to(device)
model.eval()
