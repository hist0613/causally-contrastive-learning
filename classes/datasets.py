...
class MultiCFClassifcationDataset(torch.utils.data.Dataset):
    def __init__(self, anchor_encodings, positive_encodings, negative_encodings, aux_positive_encodings, aux_negative_encodings, triplet_sample_masks, aux_triplet_sample_masks, labels):
        self.anchor_encodings = anchor_encodings
        self.positive_encodings = positive_encodings
        self.negative_encodings = negative_encodings
        self.triplet_sample_masks = triplet_sample_masks
        
        self.aux_positive_encodings = aux_positive_encodings
        self.aux_negative_encodings = aux_negative_encodings
        self.aux_triplet_sample_masks = aux_triplet_sample_masks
    ...
    def __getitem__(self, idx):
        ...
        item.update({'aux_positive_'+key: torch.tensor(val[idx]) for key, val in self.aux_positive_encodings.items()})
        item.update({'aux_negative_'+key: torch.tensor(val[idx]) for key, val in self.aux_negative_encodings.items()})
        item['aux_triplet_sample_masks'] = torch.tensor(self.aux_triplet_sample_masks[idx])
