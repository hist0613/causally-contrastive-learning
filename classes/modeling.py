...
class BertForMultiCounterfactualRobustness(BertForSequenceClassification):
    ...
    def forward(
        ...
        aux_positive_input_ids=None,
        aux_positive_attention_mask=None,
        aux_negative_input_ids=None,
        aux_negative_attention_mask=None,
        triplet_sample_masks=None,
        aux_triplet_sample_masks=None,
        lambda_weight=None,
        anchor_token_type_ids=None,
        positive_token_type_ids=None,
        negative_token_type_ids=None,
        aux_positive_token_type_ids=None,
        aux_negative_token_type_ids=None,
        position_ids=None,
        head_mask=None,
        inputs_embeds=None,
        labels=None,
        ...
        #Aux Triplet Margin Loss
        if aux_positive_input_ids is not None and aux_negative_input_ids is not None and labels is not None:
            aux_positive_outputs = self.bert(
                aux_positive_input_ids,
                attention_mask=aux_positive_attention_mask,
                token_type_ids=aux_positive_token_type_ids,
                position_ids=position_ids,
                head_mask=head_mask,
                inputs_embeds=inputs_embeds,
                output_attentions=output_attentions,
            ...
            aux_negative_outputs = self.bert(
                aux_negative_input_ids,
                attention_mask=aux_negative_attention_mask,
                token_type_ids=aux_negative_token_type_ids,
                position_ids=position_ids,
                head_mask=head_mask,
                inputs_embeds=inputs_embeds,
                output_attentions=output_attentions,
            ...
            aux_triplet_loss = None

            if lambda_weight is None:
                lambda_weight = 0.1

            if aux_triplet_sample_masks is None:
                aux_triplet_loss = triplet_loss_fct(anchor_outputs[1], aux_positive_outputs[1], aux_negative_outputs[1])
                loss = loss + lambda_weight * aux_triplet_loss
            else:
                if torch.sum(aux_triplet_sample_masks):
                    aux_triplet_loss = triplet_loss_fct(anchor_outputs[1][aux_triplet_sample_masks], aux_positive_outputs[1][aux_triplet_sample_masks], aux_negative_outputs[1][aux_triplet_sample_masks])
                    loss = loss + lambda_weight * aux_triplet_loss
